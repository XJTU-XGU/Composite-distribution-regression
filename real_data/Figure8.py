...
def plot_folds_grid_transport_map(
    results,
    fold_names,
    n_samples_kde=2000,
    n_obs=None,
    ...
    x_max=None,
    n_grid=1200,
    save_path="fold_plots_density_only/fold_grid_transport_map.pdf",
    save=True,
    show=True,
    equal_aspect=False,
):
    """
    对每个 fold 画 transport maps:
        y = \hat S(x, c)    (CSOT)
        y = \hat T(x)       (OTM)
    并与 identity map
        y = x
    ...
    # 只在 control 样本支持范围内评估，避免外插
    x_left = max(x_min, ref_min)
    x_right = min(x_max, ref_max)
    X_eval = np.linspace(x_left, x_right, n_grid)
    ...
    for fold_name in fold_names:
        ...
        Y_map_ours = np.asarray(S_hat(X_eval, c_vec), dtype=float)
        Y_map_otm = np.asarray(T_hat(X_eval), dtype=float)

        y_min_all = min(
            y_min_all,
            float(np.min(X_eval)),
            float(np.min(Y_map_ours)),
            float(np.min(Y_map_otm)),
        )
        y_max_all = max(
            y_max_all,
            float(np.max(X_eval)),
            float(np.max(Y_map_ours)),
            float(np.max(Y_map_otm)),
        )

        cached.append((fold_name, X_eval.copy(), Y_map_ours.copy(), Y_map_otm.copy()))

    pad = 0.03 * (y_max_all - y_min_all + 1e-12)
    y_lim = (y_min_all - pad, y_max_all + pad)
    ...
    for j, (fold_name, xg, yg_ours, yg_otm) in enumerate(cached):
        ax = axes[j]

        h1, = ax.plot(xg, xg, label=r"$y=x$", **style["diag"])
        h2, = ax.plot(xg, yg_ours, label=r"$\hat S(x,c)$", **style["ours"])
        h3, = ax.plot(xg, yg_otm, label=r"$\hat T(x)$", **style["otm"])
        ...
        if j == 0:
            ax.set_ylabel(r"Transported cell state")
        else:
            ax.set_ylabel("")
            ax.tick_params(axis="y", left=False, labelleft=False)

        if legend_handles is None:
            legend_handles = [h2, h3]
            legend_labels = [
                # "Identity map",
                "Transport map (our method)",
                "Transport map (Ghodrati & Panaretos (2022))",
            ]
...
if __name__ == "__main__":
    ...
    #     x_max=10,
    #     n_grid=2000,
    #     save_path="fold_plots_density_only/fold_grid_density_only.pdf",
    #     save=True,
    #     show=True,
    # )

    # 2) 新增的 transport map 图：S(x,c) vs y=x
    plot_folds_grid_transport_map(
        results=results,
        fold_names=folds,
        n_samples_kde=2000,
        n_obs=2000,
        ...
        x_max=10,
        n_grid=2000,
        save_path="outputs/Figure8.pdf",
        save=True,
        show=True,
        equal_aspect=False,   # 如果想更直观比较与对角线的偏移，可改成 True
    )
